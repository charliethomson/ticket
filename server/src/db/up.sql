CREATE TABLE users (
    ...
    brief VARCHAR(144) NOT NULL CONSTRAINT `pk_user_id` PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS devices (
    ...
    << << << < Updated upstream password TEXT << << << < Updated upstream
);
CREATE TABLE notes (
    note_id INT NOT NULL AUTO_INCREMENT,
    CONSTRAINT `pk_note_id` PRIMARY KEY (note_id),
    ...
    CONSTRAINT `fk_user_id` FOREIGN KEY (user) REFERENCES users (id) ON DELETE CASCADE ON UPDATE RESTRICT,
    posted INT NOT NULL,
    next_update INT == == == = >> >> >> > Stashed changes == == == = password TEXT >> >> >> > Stashed changes
);
